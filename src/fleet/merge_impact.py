...
def merge_impact(
    ...
    conn.executescript(
        ...
        "id TEXT PRIMARY KEY, agent_id TEXT, project TEXT, file_pattern TEXT, created_at TEXT);"
    )
    subs = conn.execute("SELECT * FROM subscriptions WHERE project = ?", (project,)).fetchall()
    for sub in subs:
        for f in files:
...
def notify_merge(
    ...
    for f in merged_files:
        ...
        # Count how many notifications were created for this file
        conn = sqlite3.connect(str(fleet_db_path))
        conn.row_factory = sqlite3.Row
        count = conn.execute(
            "SELECT COUNT(*) as c FROM notifications WHERE memory_id = ?",
            (merge_memory_id,),
        ).fetchone()["c"]
        conn.close()
