...
def test_acquire_returns_acquired(tmp_path):
    ...
    result = lock_acquire(db, "agent-a", "myproject", ["src/auth/*"], "feat/auth")
...
def test_acquire_conflicting_pattern_returns_conflict(tmp_path):
    db = tmp_path / "fleet.db"
    lock_acquire(db, "agent-a", "myproject", ["src/auth/*"], "feat/auth")
    result = lock_acquire(db, "agent-b", "myproject", ["src/auth/login.py"], "feat/login")
...
def test_no_conflict_different_paths(tmp_path):
    db = tmp_path / "fleet.db"
    lock_acquire(db, "agent-a", "myproject", ["src/auth/*"], "feat/auth")
    result = lock_acquire(db, "agent-b", "myproject", ["src/api/*"], "feat/api")
    assert result["status"] == "acquired"
...
def test_no_conflict_same_agent(tmp_path):
    db = tmp_path / "fleet.db"
    lock_acquire(db, "agent-a", "myproject", ["src/auth/*"], "feat/auth")
    result = lock_acquire(db, "agent-a", "myproject", ["src/auth/login.py"], "feat/auth2")
    assert result["status"] == "acquired"
...
def test_release_then_reacquire(tmp_path):
    db = tmp_path / "fleet.db"
    lock_acquire(db, "agent-a", "myproject", ["src/auth/*"], "feat/auth")
    rel = lock_release(db, "agent-a", "myproject")
...
def test_query_lists_active_locks(tmp_path):
    db = tmp_path / "fleet.db"
    lock_acquire(db, "agent-a", "myproject", ["src/auth/*"], "feat/auth")
    lock_acquire(db, "agent-b", "myproject", ["src/api/*"], "feat/api")
    result = lock_query(db, "myproject")
...
def test_query_filters_by_file_path(tmp_path):
    ...
    lock_acquire(db, "agent-a", "myproject", ["src/auth/*"], "feat/auth")
    lock_acquire(db, "agent-b", "myproject", ["src/api/*"], "feat/api")
    result = lock_query(db, "myproject", file_path="src/auth/login.py")
...
def test_expired_locks_cleaned_up(tmp_path):
    ...
    conn.execute(
        ...
        ("old-lock", "agent-a", "myproject", json.dumps(["src/auth/*"]), "feat/old", past, past),
    )
    conn.commit()
    conn.close()
