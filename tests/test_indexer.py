...
from fleet_mem.indexer import index_files, index_codebase, IndexFilesResult, _chunk_id
...
def test_index_files_success(mock_db, mock_embedder, tmp_path):
    ...
    with patch("fleet_mem.indexer.supported_languages", return_value=["python"]), \
         patch("fleet_mem.indexer.split_ast") as mock_split:
        
        mock_split.return_value = [
            ASTChunk(content="def hello():\n    pass", start_line=1, end_line=2, chunk_type="function", name="hello")
        ]
...
def test_index_files_batching(mock_db, mock_embedder, tmp_path):
    ...
    chunks = [TextChunk(content=f"c{i}", start_line=i, end_line=i, chunk_type="text") for i in range(70)]
...
def test_index_codebase_success(mock_db, mock_embedder, tmp_path):
    ...
    files = [
        (root / "a.py", "python", "content a"),
        (root / "b.py", "python", "content b")
    ]
    
    with patch("fleet_mem.indexer.scan_files", return_value=files), \
         patch("fleet_mem.indexer._split_file") as mock_split:
        
        mock_split.return_value = [TextChunk(content="chunk", start_line=1, end_line=1, chunk_type="text")]
...
def test_fallback_to_text_splitter(mock_db, mock_embedder, tmp_path):
    ...
    with patch("fleet_mem.indexer.supported_languages", return_value=["python"]), \
        ...
        mock_text_split.return_value = [TextChunk(content="plain text", start_line=1, end_line=1, chunk_type="text")]
